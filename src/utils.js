...
// Migrates legacy darkTheme boolean to themeMode string (runs once after an update).
// Mutates options in place, saves to sync storage if migration was needed.
function migrateThemeIfNeeded(options) {
  if ('darkTheme' in options) {
    options.themeMode = options.darkTheme ? 'dark' : 'light';
    delete options.darkTheme;
    const toSave = {};
    toSave[storedOptions] = options;
    chrome.storage.sync.set(toSave);
  }
}

// Applies dark/light class to rootElement based on themeMode.
// 'system' uses the OS preference via matchMedia.
function applyTheme(themeMode, elementId) {
  const isDark = themeMode === 'dark' ||
    (themeMode === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches);
  document.getElementById(elementId).classList.toggle('dark', isDark);
}
